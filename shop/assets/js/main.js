...
(function ($) {
    ...
    *************************************************/
    ...
    creditc();
     ...
     // RATING
    var star = 0;
    $('.cp-star').on('click', function(){
        var current_star_order = $(this).attr('value')
        if (current_star_order > star) {
            for(var i = 1; i <= current_star_order; i++ ){
                $(`[value=${i}]`).addClass('cps')
            }
        }
        if (current_star_order < star) {
            for (var j = parseInt(current_star_order)+1 ; j <= 5; j++ ){
                $(`[value=${j}]`).removeClass('cps')
            }
        }
        star = current_star_order
    });

    // COMMENT 
    $('#submit-review-btn').on("click", function (event) {
        var name = $('#basic-name').val()
        // var email = $('#basic-email').val()
        var comment = $('#basic-comment').val()
        ...
        var new_review = `<hr />
                ...
                <small>${time_now}</small> 
                    ...
                    <i class="${star >= 1?'cps':'cpr'} cp-star value="1""></i>
                    <i class="${star >= 2?'cps':'cpr'} cp-star value="2""></i>
                    <i class="${star >= 3?'cps':'cpr'} cp-star value="3""></i>
                    <i class="${star >= 4?'cps':'cpr'} cp-star value="4""></i>
                    <i class="${star >= 5?'cps':'cpr'} cp-star value="5""></i>
                </div>
                </div> <p> ${comment}</p> </div> 
                </div>`
        $('.review-inner').append(new_review)
    });
