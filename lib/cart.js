    ...
    app.post('/cart/remove', (req, res) => {
        ...
        connection.query(selectQuery, [name, userid], (error) => {
            if (error) {
                ...
                const updateQuery = 'DELETE FROM cart WHERE prod_name = ? AND userid = ?';
    ...
    app.post('/cart/incr', (req, res) => {
        ...
        connection.query(selectQuery, [name, userid], (error) => {
            if (error) {
                ...
                const updateQuery = 'UPDATE cart SET quantity = quantity + 1 WHERE prod_name = ? AND userid = ?';
                connection.query(updateQuery, [name, userid], (error) => {
                    if (error) {
                        res.status(500).send(error);
                    } else {
        ...
        connection.query(selectQuery, [name, userid], (error, results) => {
          if (error) {
            ...
            if (results[0].quantity > 1) {
              ...
              const deleteQuery = 'DELETE FROM cart WHERE prod_name = ? AND userid = ?';
