    ...
    socket.on('join', (room, otherRoom, data, time) => { 
        ...
        connection.query("SELECT line_text, line_time FROM chat_line WHERE user1 = ? AND user2 = ?", [room, otherRoom], function(err, result)
        {
            ...
            else
            {
                ...
                {
                    connection.query("INSERT INTO chat_line (user1, user2, line_text, line_time) VALUES (?, ?, ?, ?)", [room, otherRoom, '[Builder] You to ' + otherRoom + ": " + checks.message + "\n", time+"\n"], function(err, result) {
                        if (err) throw err;
                    chat.to(room).emit('chat-message', '[Builder] You to ' + otherRoom + ": " + checks.message);
                    });

                    connection.query("INSERT INTO chat_line (user1, user2, line_text, line_time) VALUES (?, ?, ?, ?)", [otherRoom, room, '[Builder] ' + room + ": " + checks.message + "\n", time+"\n"], function(err, result) {
                        if (err) throw err;
