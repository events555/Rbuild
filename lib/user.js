...
app.post('/get-service', (req, res) => {
  ...
  connection.query('SELECT * FROM users WHERE username = ?', [cookieUsername], (err, rows) => {
      ...
      {
        // check if user exists
        connection.query('SELECT * FROM users WHERE userid = ? AND role = ?', [userid, currentRole2], (err1,userExists) => {
          if (err1) {
            console.error(err1);
            return res.status(500).send('Internal server error');
          }
          ...
          {
            connection.query(`SELECT ${requiredRole}, data FROM pendingService WHERE ${currentRole} = ? AND request = ?`, [username, request], (err2, userFnd) =>
            {
              if (err2) {
                console.error(err1);
                return res.status(500).send('Internal server error');
