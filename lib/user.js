...
module.exports = function (app, connection) {
  ...
  app.post("/get-service", (req, res) => {
    ...
    currentRole2 = req.body.high_case;
    requiredRole = req.body.want_role;
          ...
          connection.query(
            ...
            (err1, userExists) => {
              ...
              if (username !== cookieUsername) {
                ...
                connection.query(
                  `SELECT ${requiredRole}, data FROM pendingService WHERE ${currentRole} = ? AND request = ?`,
                  [username, request],
                  (err2, userFnd) => {
                    if (err2) {
                      console.error(err1);
                      ...
                      return res.status(200).json(userFnd);
                    }
                  }
                );
              }
          ...
          connection.query(
            ...
            (err1, userExists) => {
              ...
              if (username !== cookieUsername) {
                ...
                connection.query(
                  `SELECT * FROM pendingService WHERE customer = ? AND request = ?`,
                  [username, request],
                  (err2, userFnd) => {
                    if (err2) {
                      console.error(err1);
          ...
          connection.query(
            ...
            (err1, userExists) => {
              ...
              if (username !== cookieUsername) {
                ...
                connection.query(
                  "SELECT username, online FROM users WHERE role = ?",
                  [request],
                  (err, results) => {
                    if (err) {
                      // Handle the error
                      ...
                      connection.query(
                        "SELECT employee, COUNT(*) AS count FROM pendingService WHERE employee IN (?) GROUP BY employee",
                          ...
                          if (err) {
                            ...
                            for (let result of results) {
                              ...
                              let index = users.findIndex(
                                (user) => user.username === result.employee
                              );
                      ...
                      */
                            ...
                            if (employeeUsername.username) {
                              ...
                              const date = new Date()
                                .toISOString()
                                .slice(0, 10); // Get the current date in YYYY-MM-DD format
                              connection.query(
                                "INSERT INTO pendingService (employee, customer, request, date, data) VALUES (?, ?, ?, ?, ?)",
                                [
                                ...
                                function (error) {
                                  ...
                                  res
                                    .status(201)
                                    .send(
                                      "Request successfully created with " +
                                        employeeUsername.username
                                    );
                                }
                              );
                              ...
                              res
                                .status(400)
                                .send("No employee found for the request");
                          }
                        }
                      );
                    }
  ...
  app.get("/view_orders", (req, res) => {
    res.sendFile(path.join(__dirname, "../public", "purchase_manage.html"));
  });
  ...
  app.get("/ticket", (req, res) => {
    res.sendFile(path.join(__dirname, "../public", "request_form.html"));
  });
};
